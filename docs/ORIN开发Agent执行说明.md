...
> 场景：支持“夜间无人值守连续开发”（例如 OpenClaw / MiniMax 2.5 这类会话式开发 agent）
...
当用户离线（例如夜间）时，本说明默认进入“无人值守模式”，你必须持续推进，不要把大多数选择题抛回给用户。

### 0.1.1 默认行为：先做可逆决策，再继续执行

- 遇到非高风险不确定项：直接做“最小可逆”选择并继续
- 选择已有实现风格和目录，不额外引入新范式
- 优先补闭环，不做大规模重构
- 所有关键假设写入最终报告，不因假设而停机等待

### 0.1.2 只有这 4 类问题允许阻塞提问

仅当出现以下情形，才允许停下来等待用户答复：

1. 破坏性不可逆操作（批量删除数据、重置、覆盖历史）
2. 安全或合规高风险操作（密钥、权限边界、鉴权策略重大调整）
3. 成本高且不可回收的外部动作（付费调用、大规模生产写入）
4. 存在两条以上路线，且后果差异显著、无法通过“可逆默认值”规避

除以上 4 类外，禁止因为“偏好不明确”而停止推进。

### 0.1.3 禁止等待式对话

在无人值守模式下，禁止输出以下类型的话术作为中断：
...
- 直接按默认方案执行
- 在结果中列出“已采用假设”和“可选替代方案”

### 0.1.4 默认决策顺序（冲突时按序裁决）
...
如果因环境、密钥或外部服务不可达而无法端到端联调，不要停工，按以下顺序继续：

1. 先补本地可验证的核心逻辑
2. 增加明确的降级、错误语义和日志
3. 补最小测试或 smoke 脚本（可 mock）
4. 在报告中标记“外部依赖阻塞项”和复现方式

目标是“先把可做完的全部做完”，不是“先提问再做”。
...
- 如果影响范围不完整，先按“最小安全范围”改动并验证，再逐步扩展
- 不因边界不清而暂停；优先提交可验证的子闭环
...
- 若 15 分钟内无法完全确认类型，先按“最可能类型 + 可逆实现”推进
- 结果中显式写出判断依据和剩余不确定点
...
- 至少做“可在当前环境执行”的最低验证，不允许因缺少完美环境而零验证交付
- 对无法验证部分给出可直接执行的验证命令和前置条件
...
- [ ] 本次是否存在“本可继续却停下来等待”的情况（如有，视为不合格）
- [ ] 所有关键假设是否已写明，且给出替代方案
- [ ] 所有阻塞项是否都附带“可复现命令/日志/缺失条件”
- [ ] 是否至少交付一个可验证增量，而非只给分析结论
