# ORIN 文档索引

`docs/` 已按“只保留当前可信基线”做过一轮强收缩。下面这批文档是当前建议保留并继续维护的最小集合。

## 当前保留文档

- [ORIN开发Agent执行说明.md](./ORIN开发Agent执行说明.md)：给开发 agent 的执行说明（含“夜间无人值守连续开发”规则）
- [ORIN统一开发总计划.md](./ORIN统一开发总计划.md)：后续开发、排期、验收的统一总计划
- [阶段0_改造基线.md](./阶段0_改造基线.md)：当前真实架构与改造范围基线
- [系统功能实现评估报告.md](./系统功能实现评估报告.md)：当前能力边界与保守交付口径
- [原始设计与当前实现对照.md](./原始设计与当前实现对照.md)：原始设计与当前代码的差异说明
- [API文档.md](./API文档.md)：当前接口入口与查阅方式
- [统一API用户文档.md](./统一API用户文档.md)：外部调用方统一入口与快速接入说明
- [使用指南.md](./使用指南.md)：当前前端入口与联调方式
- [部署指南.md](./部署指南.md)：部署与本地环境说明
- [I1.2-部署前检查清单.md](./I1.2-部署前检查清单.md)：部署前检查清单
- [../orin-frontend/docs/README.md](../orin-frontend/docs/README.md)：前端专项文档入口（重构灰度/发布 checkpoint/组件文档）

## 阅读顺序

建议按下面顺序阅读：

1. [ORIN统一开发总计划.md](./ORIN统一开发总计划.md)
2. [ORIN开发Agent执行说明.md](./ORIN开发Agent执行说明.md)
3. [阶段0_改造基线.md](./阶段0_改造基线.md)
4. [系统功能实现评估报告.md](./系统功能实现评估报告.md)
5. [API文档.md](./API文档.md)
6. [统一API用户文档.md](./统一API用户文档.md)
7. [部署指南.md](./部署指南.md)
8. [I1.2-部署前检查清单.md](./I1.2-部署前检查清单.md)
9. [使用指南.md](./使用指南.md)
10. [原始设计与当前实现对照.md](./原始设计与当前实现对照.md)

如果你准备把任务交给远端开发 agent 夜间连续执行，建议把第 1、2 步互换，先读执行说明再读总计划：

1. [ORIN开发Agent执行说明.md](./ORIN开发Agent执行说明.md)
2. [ORIN统一开发总计划.md](./ORIN统一开发总计划.md)

## 维护原则

- 如果一个文档不能帮助当前开发、联调、部署或验收，就不保留。
- 如果一个文档与当前代码冲突，应优先修正文档或删除文档。
- 不再保留“完成度过满”“页面存在即视为交付”的文档。
