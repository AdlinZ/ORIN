...
A: 检查 `orin-backend` 日志中的连接信息。如果使用 Dify，请确保 Dify API Key 具有知识库读写权限，且 Dify 服务正常运行。系统会自动重试失败的任务。
