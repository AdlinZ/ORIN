...
这份文档不再尝试手写“完整接口清单”，而是提供当前仓库中最稳定的接口分组和查阅方式。具体参数、请求体和响应结构，请以 OpenAPI 和控制器代码为准。
...
| 智能体 | `/api/v1/agents/*` | 智能体接入、列表、交互、管理 |
| API 网关 | `/v1/*` | OpenAI 兼容网关 |
| 知识库 | `/api/v1/knowledge/*` | 知识库、文档、检索、同步 |
| 工作流 | `/api/workflows/*`, `/api/v1/workflow/*` | 工作流管理与代理执行 |
| 协作 | `/api/v1/collaboration/*` | 协作任务、包、编排、事件 |
| Trace | `/api/traces/*` | Trace 查询、统计、深链 |
| 观测 | `/api/v1/observability/*` | 观测配置与状态 |
| 系统设置 | `/api/v1/system/*` | 日志、邮件、配置、维护 |
| 用户权限 | `/api/v1/users/*`, `/api/v1/roles/*`, `/api/v1/departments/*` | 系统管理 |
...
- `GET /api/v1/knowledge/sync/changes`
...
- `POST /api/v1/collaboration/orchestrate`
...
```bash
rg -n "/api/v1/workflow/run|/api/v1/agents|/api/v1/knowledge" \
  orin-backend/src/main/java/com/adlin/orin
```
