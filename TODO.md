...
- [x] `P1` 统一智能体、知识库的归属、状态、权限与审计语义
  ...
  - 语义口径写入 [docs/角色矩阵.md](docs/角色矩阵.md)；资源级 ACL 已按归属过滤列表与读写
- [x] `P1` 收敛智能体列表/接入/工作台和知识中心/资产/检索实验室的重复入口
  - 智能体菜单：运维工作台 / 列表 / 会话记录；调试工作台与接入保留深链
  - 知识库菜单：知识资产 / 知识检索；`center` 继续重定向到检索实验室
...
- [x] `P1` 普通用户默认进入 `/portal`，AI 应用管理员（`ROLE_OPERATOR`）默认进入运维工作台，平台管理员默认进入监控总览
...
- [~] `P1` 角色专属视图：运维工作台已闭环；门户露出「我的 API Key」摘要并弱化管理端文案；管理员总览补用户/API Key/24h 审计密度（配额 Top10 与限流告警仍后续）
- [x] `P2` 资源级 ACL（智能体/知识库/工作流管理面）：非管理员按 `owner_user_id` 过滤列表与读写；管理员全局可见；跨用户共享/委托仍待后续
...
- [x] `P1` 为前端显示补齐状态标签、失败原因、最后处理时间
  - 解析使用 `parseError`，向量化使用 V91 新增的 `vectorError`；Provider 原始响应和凭据不持久化
