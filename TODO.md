...
- [x] **Milvus 向量引擎**：已集成 Milvus 向量引擎，支持知识库的增删改查及向量检索。
- [x] **修复知识库删除**：已解决因 Milvus 不在线导致的删除挂起和 500 错误。
- [x] **修复计数显示**：知识库列表现在实时计算文档数量。

## 🛠️ 环境配置 (重要)

- [x] **配置 Docker 版 Milvus**：已安装并配置好 Milvus 向量引擎，可正常连接使用。

## 💻 功能增强

- [ ] **新增：向量数据库配置页面**（可选功能）：
  - [ ] 在前端"系统设置"中增加"向量检索配置"子项。
  - [ ] 支持在线修改 Milvus 地址 (`host`)、端口 (`port`) 和 Token（可选）。
  - [ ] 增加"联通性检查"按钮，点击后后端尝试 ping Milvus 并返回状态。
- [ ] **切换真实/Mock 模式**：
  - [ ] 优化后端配置，支持从 `application.properties` 或数据库动态切换向量引擎（可选功能）。
...
- [x] **项目文档更新**：移除 Milvus/MinIO/etcd 相关内容，改为 Mock 模式
