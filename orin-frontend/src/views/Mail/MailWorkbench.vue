  ...
  <div class="mail-workbench gmail-layout">
    ...
    >
      <template #actions>
        ...
        <el-button class="top-setting-btn" @click="switchModule('service')">
          服务配置
        </el-button>
      </template>
      <template #filters>
    ...
    <div class="gmail-main">
      <aside class="gmail-sidebar">
        <el-button class="compose-btn" type="primary" @click="switchModule('compose')">
          撰写
        </el-button>
        ...
        <button class="nav-item" :class="{ active: activeTab === 'templates' }" @click="switchModule('templates')">
          模板
        </button>
        <button class="nav-item" :class="{ active: activeTab === 'service' }" @click="switchModule('service')">
          邮箱设置
        </button>
        ...
        <div v-if="!imapConfigured" class="sidebar-tip">
          请先在“邮箱设置”完成 IMAP 配置
        </div>
      </aside>
      ...
      <section class="gmail-content">
        <template v-if="activeTab === 'inbox'">
          <div class="list-toolbar">
            <div class="toolbar-left">
              ...
              <el-button text :disabled="!imapConfigured || selectedMailIds.length === 0" @click="batchMarkRead">
                标记已读
              </el-button>
              <el-button
                text
                type="danger"
                :disabled="!imapConfigured || selectedMailIds.length === 0"
                @click="batchDeleteMail"
              >
                删除
              </el-button>
            </div>
            <div class="toolbar-right">
              <el-tag type="info">
                已选 {{ selectedMailIds.length }}
              </el-tag>
              <el-tag :type="mailConnected ? 'success' : 'warning'">
                {{ mailConnected ? '邮件服务已连接' : '邮件服务未连接' }}
              </el-tag>
            </div>
          </div>
            ...
            <div class="mail-list" :class="{ 'is-empty': !loadingInbox && filteredInboxList.length === 0 }">
              ...
              >
                <div class="mail-row-left">
                  ...
                  <button class="star-btn" @click.stop="toggleStar(mail)">
                    {{ mail.starred ? '★' : '☆' }}
                  </button>
                </div>
              ...
              <div v-if="!loadingInbox && !imapConfigured" class="mail-empty">
                <div class="mail-empty-card">
                  ...
                  <div class="mail-empty-actions">
                    <el-button type="primary" @click="switchModule('service')">
                      去配置 IMAP
                    </el-button>
                  </div>
                </div>
              ...
              <div v-else-if="!loadingInbox && filteredInboxList.length === 0" class="mail-empty">
                <div class="mail-empty-card">
                  ...
                  <div class="mail-empty-desc">
                    可以先拉取最新邮件，或更换筛选条件查看历史记录。
                  </div>
                  <div class="mail-empty-actions">
                    <el-button type="primary" :loading="fetchingInbox" @click="fetchInboxAction">
                      拉取新邮件
                    </el-button>
                  </div>
                </div>
            ...
            <div class="mail-reader">
              <template v-if="selectedInboxMail">
                <div class="reader-header">
                  ...
                  <div class="reader-actions">
                    <el-button size="small" :disabled="!isInboxUnread(selectedInboxMail)" @click="markInboxRead(selectedInboxMail)">
                      已读
                    </el-button>
                    <el-button size="small" type="danger" @click="deleteInboxMailAction(selectedInboxMail)">
                      删除
                    </el-button>
                  </div>
                </div>
              ...
              <div v-else class="reader-empty">
                <div class="reader-empty-card">
                  <div class="reader-empty-title">
                    {{ imapConfigured ? '选择一封邮件开始阅读' : '先完成 IMAP 配置后再查看邮件详情' }}
                  </div>
                  <div class="mail-empty-desc">
        ...
        <template v-else-if="activeTab === 'compose'">
          <div class="module-sheet compose-sheet">
            ...
            <div class="compose-actions">
              ...
              <el-button @click="triggerAttachmentPicker">
                附件
              </el-button>
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item>
                      <el-checkbox v-model="sendForm.separateSend">
                        分别发送（逐个收件人）
                      </el-checkbox>
                    </el-dropdown-item>
                  </el-dropdown-menu>
              ...
              <el-button @click="applyDefaultTemplate">
                套用默认模板
              </el-button>
            </div>
            <el-form :model="sendForm" label-width="90px" class="compose-form compose-qq-form">
              <el-form-item label="收件人" required>
                <div class="recipient-row">
                  ...
                  <el-button text @click="showCc = !showCc">
                    抄送
                  </el-button>
                  <el-button text @click="showBcc = !showBcc">
                    密送
                  </el-button>
                </div>
              </el-form-item>
              ...
              <el-form-item v-if="templateVariables.length > 0" label="变量">
                <div class="variables-grid">
                  <el-input
                    ...
                    :key="field"
                    v-model="sendForm.variables[field]"
                    :placeholder="field"
                  />
                </div>
              </el-form-item>
              <el-form-item label="内容" required class="editor-form-item">
                <div class="qq-editor-wrap">
                  <div class="editor-toolbar">
                    <el-button text @click="execEditorCommand('bold')">
                      B
                    </el-button>
                    <el-button text @click="execEditorCommand('italic')">
                      <i>I</i>
                    </el-button>
                    <el-button text @click="execEditorCommand('underline')">
                      <u>U</u>
                    </el-button>
                    <el-divider direction="vertical" />
                    <el-button text @click="execEditorCommand('insertUnorderedList')">
                      • 列表
                    </el-button>
                    <el-button text @click="execEditorCommand('insertOrderedList')">
                      1. 列表
                    </el-button>
                ...
                <div class="attachment-block">
                  ...
                  <div v-if="attachments.length > 0" class="attachment-list">
                    <div v-for="(item, idx) in attachments" :key="`${item.name}-${idx}`" class="attachment-item">
                      ...
                      <el-button text type="danger" @click="removeAttachment(idx)">
                        移除
                      </el-button>
                    </div>
                  </div>
        ...
        <template v-else-if="activeTab === 'tracking'">
          <div class="module-sheet">
            ...
            <div class="tab-actions">
              ...
              <el-button :loading="loadingLogs" @click="loadLogs">
                刷新
              </el-button>
              <el-button
                type="warning"
                :disabled="selectedFailedLogs.length === 0"
                :loading="retryingBatch"
                @click="batchRetryLogs"
              >
        ...
        <template v-else>
          <div class="module-sheet">
            ...
            <div class="tab-actions">
              <el-button type="primary" :loading="savingConfig" @click="saveMailConfigAction">
                保存配置
              </el-button>
              <el-input
                v-model="testEmail"
                ...
                clearable
              />
              <el-button :loading="testingConfig" :disabled="!mailConnected && !hasConfigDraft" @click="testMailConfigAction">
                发送测试邮件
              </el-button>
            </div>
            ...
            <el-form :model="mailConfigForm" label-width="140px" class="config-form">
              <el-form-item label="发送方式" required>
                <el-radio-group v-model="mailConfigForm.mailerType">
                  <el-radio-button value="mailersend">
                    MailerSend API
                  </el-radio-button>
                  <el-radio-button value="resend">
                    Resend
                  </el-radio-button>
                  <el-radio-button value="smtp">
                    SMTP
                  </el-radio-button>
                </el-radio-group>
              </el-form-item>
              ...
              <template v-else>
                <el-form-item label="SMTP Host" required>
                  <el-input v-model="mailConfigForm.smtpHost" placeholder="smtp.example.com" />
                </el-form-item>
                <el-form-item label="SMTP Port" required>
                  <el-input-number v-model="mailConfigForm.smtpPort" :min="1" :max="65535" />
                </el-form-item>
                <el-form-item label="用户名">
                  <el-input v-model="mailConfigForm.smtpUsername" placeholder="SMTP 用户名" />
                </el-form-item>
              ...
              <el-form-item label="发件邮箱" required>
                <el-input v-model="mailConfigForm.fromEmail" placeholder="noreply@example.com" />
              </el-form-item>
    ...
    <el-dialog v-model="templateDialogVisible" :title="templateEditing.id ? '编辑模板' : '新建模板'" width="680px">
      <el-form :model="templateEditing" label-width="100px">
        <el-form-item label="模板名称" required>
          <el-input v-model="templateEditing.name" />
        </el-form-item>
        <el-form-item label="模板编码" required>
          <el-input v-model="templateEditing.code" :disabled="Boolean(templateEditing.id)" />
        </el-form-item>
        <el-form-item label="邮件主题" required>
          <el-input v-model="templateEditing.subject" />
        </el-form-item>
        <el-form-item label="模板内容" required>
          <el-input v-model="templateEditing.content" type="textarea" :rows="10" />
        </el-form-item>
        <el-form-item label="默认模板">
          <el-switch v-model="templateEditing.isDefault" />
        </el-form-item>
        <el-form-item label="启用">
          <el-switch v-model="templateEditing.enabled" />
        </el-form-item>
      </el-form>
      <template #footer>
        ...
        <el-button type="primary" :loading="savingTemplate" @click="saveTemplateAction">
          保存
        </el-button>
      </template>
    </el-dialog>
