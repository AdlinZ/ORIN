...
const messages = {
  "en-US": {
    ...
    brand: { subtitle: "Multi-agent orchestration and trace system" },
    ...
    workflow: {
      ...
      template_desc_router_specialists: "Router selects the best specialist for the user intent, then optionally passes through a finalizer.",
      template_desc_planner_executor: "Planner decomposes the request into sub-tasks, delegates each task to workers, then synthesizes a final answer.",
      template_desc_supervisor_dynamic: "Supervisor decides delegation at runtime, loops through workers as needed, and composes the final answer.",
      template_desc_single_agent_chat: "Direct chat with one selected agent using a minimal start -> agent -> end graph.",
      template_desc_peer_handoff: "Router selects the first owner, then specialists hand work to each other inside a shared collaboration zone.",
    },
    chat: {
      title: "Run",
      active: "Active setup",
      noneSelected: "No setup selected",
      editWorkflow: "Edit Setup",
      clear: "Clear",
  ...
  "zh-CN": {
    ...
    workflow: {
      ...
      template_desc_router_specialists: "先由 Router 选择最匹配的专家，再按需经过 Finalizer 统一收口。",
      template_desc_planner_executor: "先由 Planner 拆解任务，再分配给 Worker 执行，最后统一合成答复。",
      template_desc_supervisor_dynamic: "由 Supervisor 在运行中动态委派，按需循环调度 Worker 并收敛结果。",
      template_desc_single_agent_chat: "直接与一个选定的 Agent 对话，图结构最简。",
      template_desc_peer_handoff: "先选出第一责任人，再由专家之间在共享协作区内相互交接，直到任务收敛。",
    },
    chat: {
      title: "运行",
      active: "当前方案",
...
export function createUiI18n() {
  ...
  function workflowTypeDesc(type, fallback = "") {
    ...
    const fallbacks = {
      router_specialists: "Router selects the best specialist for the user intent, then optionally passes through a finalizer.",
      planner_executor: "Planner decomposes the request into sub-tasks, delegates each task to workers, then synthesizes a final answer.",
      supervisor_dynamic: "Supervisor decides delegation at runtime, loops through workers as needed, and composes the final answer.",
      single_agent_chat: "Direct chat with one selected agent using a minimal start -> agent -> end graph.",
      peer_handoff: "Router selects the first owner, then specialists hand work to each other inside a shared collaboration zone.",
    }
    return fallbacks[type] || fallback
  }

  return { locale, setLocale, t, workflowTypeLabel, workflowTypeDesc }
}
