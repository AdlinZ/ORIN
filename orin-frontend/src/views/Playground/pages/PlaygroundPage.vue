...
<template>
  <div class="playground-shell">
    ...
    <aside v-if="leftVisible" class="playground-col-left">
      <div class="playground-sidebar-tabs">
        <button
          type="button"
          class="playground-sidebar-tab"
          :class="{ active: sidebarTab === 'graph' }"
          @click="sidebarTab = 'graph'"
        >
          工作流图
        </button>
        <button
          type="button"
          class="playground-sidebar-tab"
          :class="{ active: sidebarTab === 'trace' }"
          @click="sidebarTab = 'trace'"
        >
          执行轨迹
        </button>
      </div>
      ...
      <GraphViewer
        v-if="sidebarTab === 'graph'"
        :graph="props.selectedGraph"
        :active-node-id="props.activeNodeId"
        :trace="props.trace"
        :trace-playing="props.tracePlaying"
      />

      <TraceViewer
        v-else
        :trace="props.trace"
        :playing="props.tracePlaying"
      />
    </aside>
