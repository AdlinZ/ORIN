...
function layoutFallback(nodes, width, height) {
  ...
  if (mergeNode) result.push({ ...mergeNode, x: width / 2, y: height * 0.75 });
...
<template>
  <section class="glass-panel graph-shell" :class="{ 'graph-shell-running': props.tracePlaying }">
    <header class="run-panel-header">
      <h3 class="run-panel-title">
        <Network :size="18" class="text-blue-500" />
        工作流图
      </h3>
      <span class="chip chip-blue run-status-chip">
        <span class="status-dot" :class="{ 'is-running': props.tracePlaying }"></span>
        {{ props.tracePlaying ? t("trace.playing") : t("graph.live") }}
      </span>
    </header>
