<template>
  <div class="gateway-routes-tab">
    <div class="section-card">
      ...
      <div v-if="mode === 'actions'" class="route-action-panel">
        <div>
          <strong>创建模型 API、后台控制面或服务代理入口</strong>
          <p>使用同一张表单配置路径匹配、目标服务、直连地址、认证要求和入口级策略。</p>
        </div>
      </div>
    ...
    <!-- Create/Edit Dialog -->
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑入口' : '添加入口'" width="760px">
      <el-form :model="form" label-width="110px">
        ...
        <el-alert type="info" :closable="false" style="margin-bottom:14px" v-if="isLocalFormRoute">
          <template #title>
            <strong>本地策略入口 (LOCAL)</strong>
          </template>
          目标地址和关联服务均为空时，此入口为本地策略入口：请求不会被代理转发，
          而是经过 ACL → 认证 → 入口级限流 → 审计日志 后，直接交给 ORIN 本地 Controller 处理。
        ...
        <el-form-item label="直连地址">
          <el-input v-model="form.targetUrl" placeholder="http://localhost:8080/api（均不填则为本地策略入口）" :disabled="!!form.serviceId" />
        </el-form-item>
        <el-row :gutter="10">
          <el-col :span="12">
    ...
    <!-- Test Route Dialog -->
    <el-dialog v-model="testDialogVisible" title="测试入口匹配" width="500px">
      ...
      <el-alert v-if="testResult" :type="testResult.success ? 'success' : 'warning'" :closable="false" style="margin-top:12px">
        <template #title>
          <span v-if="testResult.success">
            匹配入口：<strong>{{ testResult.matchedRoute }}</strong>
            <el-tag size="small" style="margin-left:8px">{{ testResult.routeType || 'PROXY' }}</el-tag>
            → {{ testResult.targetUrl || 'ORIN 本地 Controller' }}
          </span>
          <span v-else>无匹配入口</span>
        </template>
      </el-alert>
      <template #footer>
...
/** 表格行判断：本地入口（无 targetUrl 且无 serviceId） */
...
const deleteRoute = async (row) => {
  try {
    await ElMessageBox.confirm(`确定要删除入口 "${row.name}" 吗?`, '提示', { type: 'warning' })
...
const toggleRoute = async (row) => {
  try {
    ...
    ElMessage.success(row.enabled ? '入口已启用' : '入口已禁用')
  } catch (e) {
