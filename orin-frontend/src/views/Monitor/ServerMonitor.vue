      ...
      <div class="workspace-session-pane">
        ...
        <div v-if="sessionPaneCollapsed" class="collapsed-pane">
          <el-button
            ...
            circle
            :icon="Monitor"
            @click="sessionPaneCollapsed = false"
          />
        </div>
        ...
        <template v-else>
          ...
          <div class="session-list">
            <button
              type="button"
              :class="['session-item', 'session-overview-entry', { active: !currentServerId }]"
            ...
            >
              ...
              <el-dropdown
                trigger="click"
                @command="cmd => handleNodeAction(cmd, node)"
                @click.stop
              >
      ...
      <div class="messages-container" v-loading="loading">
        <div class="dashboard-content">
          <section class="runtime-command-panel">
            ...
            <div class="runtime-command-body">
              <div class="runtime-command-copy">
                <span class="section-kicker">Runtime Monitor</span>
                <h3>服务器运行态总览</h3>
                <p class="runtime-command-desc">
                  {{ normalizedNodes.length }} 个节点 · {{ attentionNodeCount }} 个需关注 · 最近
                  {{ latestSnapshotTime }}
                </p>
              </div>
              ...
              <div class="runtime-signal-grid">
                <div class="runtime-signal">
                  <span>节点总数</span>
                  <strong>{{ normalizedNodes.length }}</strong>
                </div>
                <div class="runtime-signal">
                  <span>在线节点</span>
                ...
                <div class="runtime-signal">
                  <span>已配置</span>
                  <strong>{{ configuredNodeCount }}</strong>
                </div>
                <div class="runtime-signal">
                  <span>需关注</span>
                  <strong>{{ attentionNodeCount }}</strong>
                </div>
                <div class="runtime-signal">
                  <span>离线</span>
                  <strong>{{ offlineNodeCount }}</strong>
                </div>
                <div class="runtime-signal">
                  <span>待采集</span>
                  <strong>{{ pendingNodeCount }}</strong>
                </div>
                <div class="runtime-signal">
                  <span>刷新间隔</span>
                  <strong>{{ prometheusConfig.refreshInterval || 15 }}s</strong>
                </div>
                <div class="runtime-signal wide">
                  <span>最近采集</span>
              ...
              <div class="runtime-health-panel">
                <div class="health-ring" :style="{ '--health': `${onlineRate}%` }">
                  <span>{{ onlineRate }}%</span>
                </div>
                <div class="health-copy">
                  <span>在线率</span>
                  <strong>{{ onlineNodeCount }} / {{ normalizedNodes.length }}</strong>
                </div>
              </div>
            </div>
          </section>
          ...
          <div class="panel-section margin-bottom-lg">
            <section class="nodes-overview-section">
              <div class="section-toolbar">
                ...
                <span class="section-hint">点击节点卡片查看详细指标</span>
              </div>
              ...
              <div v-if="overviewNodes.length" class="node-grid">
                ...
                >
                  ...
                  <div class="node-summary-tags">
                    <span>{{ node.configured ? '专属数据源' : '全局数据源' }}</span>
                    <span>{{
                      node.publicIp ||
                      node.publicIP ||
                      node.privateIp ||
                  ...
                  <div class="node-summary-metrics">
                    <div
                      ...
                      :style="{
                        '--metric-progress': `${clampPercent(node.snapshot.cpuUsage)}%`,
                        '--metric-color': getUsageColor(node.snapshot.cpuUsage)
                      }"
                    >
                      <span>CPU</span>
                    ...
                    <div
                      class="summary-metric"
                      :style="{
                        '--metric-progress': `${clampPercent(node.snapshot.memoryUsage)}%`,
                        '--metric-color': getUsageColor(node.snapshot.memoryUsage)
                      }"
                    >
                      <span>内存</span>
                    ...
                    <div
                      class="summary-metric"
                      :style="{
                        '--metric-progress': `${clampPercent(node.snapshot.diskUsage)}%`,
                        '--metric-color': getUsageColor(node.snapshot.diskUsage)
                      }"
                    >
                      <span>磁盘</span>
                    ...
                    <div
                      class="summary-metric"
                      :style="{
                        '--metric-progress': `${clampPercent(node.snapshot.gpuUsage)}%`,
                        '--metric-color': getUsageColor(node.snapshot.gpuUsage)
                      }"
                    >
                      <span>GPU</span>
                  ...
                  <div class="node-summary-foot">
                    <span>{{ node.id === 'local' ? '本地主节点' : '远程节点' }}</span>
                    <span>{{
                      formatDateTime(node.snapshot.recordedAt || node.snapshot.timestamp, true)
                    }}</span>
                  </div>
                </button>
              </div>
          ...
          <section
            v-if="!hasSelectedNode && overviewNodes.length"
            class="runtime-insight-strip panel-section"
          >
            <div class="runtime-insight-item">
              <el-icon><Clock /></el-icon>
              <div>
                <span>采集状态</span>
                <strong>{{ onlineNodeCount }} 正常 · {{ pendingNodeCount }} 待采集</strong>
              </div>
            </div>
            <div class="runtime-insight-item">
              <el-icon><Warning /></el-icon>
              <div>
                <span>关注项</span>
                <strong>{{ offlineNodeCount }} 离线 · {{ highUsageNodeCount }} 高负载</strong>
              </div>
            </div>
            <div class="runtime-insight-item">
              <el-icon><Monitor /></el-icon>
            ...
            <el-row :gutter="20" class="margin-bottom-lg">
              <el-col :xs="24" :sm="12" :lg="6">
                <el-card shadow="never" class="metric-card cpu-card">
                  ...
                  <div class="metric-label">
                    型号: {{ selectedServerInfo.cpuModel || 'Unknown' }}
                  </div>
                </el-card>
              </el-col>
              ...
              <el-col :xs="24" :sm="12" :lg="6">
                <el-card shadow="never" class="metric-card memory-card">
                  ...
                  <div class="metric-label">
                    ...
                    {{
                      memoryInfo.total > 0
                        ? `${formatBytes(memoryInfo.available)} (${formatPercent(100 - memoryInfo.percent)})`
                        : 'N/A'
                    }}
                  </div>
                </el-card>
              ...
              <el-col :xs="24" :sm="12" :lg="6">
                <el-card shadow="never" class="metric-card disk-card">
                  ...
                  <div class="metric-label">
                    可用: {{ diskInfo.total > 0 ? formatBytes(diskInfo.available) : 'N/A' }}
                  </div>
                </el-card>
              </el-col>
                ...
                <el-card shadow="never" class="metric-card gpu-card">
                  ...
                  <div class="metric-sub">
                    <span
                      >显存:
                      {{
                        selectedSnapshot.gpuMemory ||
                        `${formatBytesOrNA(gpuInfo.memoryUsed)} / ${formatBytesOrNA(gpuInfo.memoryTotal)}`
                      }}</span
                    >
                  </div>
                  <div class="metric-gauge">
                  ...
                  <div class="server-info-grid">
                    ...
                    <div class="info-item">
                      ...
                      <span class="info-value"
                        >{{ formatLogicalCores(selectedServerInfo) }} 线程</span
                      >
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                      ...
                      <span class="info-value">{{
                        formatCount(selectedServerInfo.processCount)
                      }}</span>
                    </div>
                    ...
                    <div class="info-item">
                      ...
                      <span class="info-value">{{
                        selectedNode?.prometheusUrl || '全局默认'
                      }}</span>
                    </div>
                    <div class="info-item">
                      <span class="info-label">公网 IP</span>
                      <span class="info-value">{{
                        selectedNode?.publicIp || selectedNode?.publicIP || 'N/A'
                      }}</span>
                    </div>
                    <div class="info-item">
                      <span class="info-label">私网 IP</span>
                      <span class="info-value">{{
                        selectedNode?.privateIp || selectedNode?.privateIP || 'N/A'
                      }}</span>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                      ...
                      <span class="info-value">{{
                        selectedNode?.instanceFamily || selectedNode?.flavor || 'N/A'
                      }}</span>
                    </div>
                    <div class="info-item wide">
                      <span class="info-label">镜像</span>
                      <span class="info-value">{{
                        selectedNode?.image || selectedNode?.imageName || 'N/A'
                      }}</span>
                    </div>
                    <div class="info-item">
                      ...
                      <span class="info-value">{{
                        formatDateTime(
                          selectedNode?.expireAt ||
                            selectedNode?.expiredAt ||
                            selectedNode?.expireTime
                        )
                      }}</span>
                    </div>
                  </div>
              ...
              <template #header>
                <div class="card-header">
                  ...
                  <span class="record-count"
                    >当前节点 {{ currentServerName }}，共 {{ historyTotal }} 条</span
                  >
                  <el-button
                    type="primary"
                    size="small"
        ...
        <div class="node-form-section">
          ...
          <div class="section-grid">
            <el-form-item label="节点 ID" prop="serverId" required>
              ...
              >
                <template #append>
                  <el-button :disabled="isEditingNode" @click="regenerateNodeId"
                    >重新生成</el-button
                  >
                </template>
              </el-input>
    ...
    >
      <el-form :model="prometheusConfig" label-position="top" class="config-form">
        ...
        <el-form-item v-if="prometheusConfig.enabled" label="Prometheus 服务器地址">
          <el-input
            v-model="prometheusConfig.prometheusUrl"
            placeholder="例如: http://192.168.1.107:9090"
          />
          <div class="form-tip">
            这里配置的是全局默认 Prometheus 地址；单节点如果配置了专属
            URL，会优先使用节点自己的地址。
          </div>
        </el-form-item>
        ...
        <div class="config-strategy-grid">
          <div class="strategy-item">
            ...
            <div class="inline-setting">
              <el-input-number
                v-model="prometheusConfig.cacheTtl"
                :min="5"
                :max="300"
                :step="5"
                size="small"
          ...
          <div class="strategy-item">
            ...
            <div class="inline-setting">
              <el-input-number
                v-model="prometheusConfig.refreshInterval"
                :min="5"
                :max="300"
                :step="5"
                size="small"
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="configSaving" @click="savePrometheusConfig"
          >保存</el-button
        >
      </template>
    </el-dialog>
...
const canSubmitNodeForm = computed(
  () => Boolean((nodeForm.value.serverId || '').trim()) && nodePrometheusUrlValid.value
)
...
const filteredNodes = computed(() => {
  ...
  return normalizedNodes.value.filter(
    node =>
...
const offlineNodeCount = computed(
  () => overviewNodes.value.filter(node => node.snapshot.online === false).length
)

const pendingNodeCount = computed(
  () => overviewNodes.value.filter(node => !node.snapshot || node.snapshot.online === null).length
)

const highUsageNodeCount = computed(
  () =>
    overviewNodes.value.filter(node => {
      const snapshot = node.snapshot || {}
      return [snapshot.cpuUsage, snapshot.memoryUsage, snapshot.diskUsage, snapshot.gpuUsage].some(
        value => clampPercent(value) >= 80
      )
    }).length
)
  ...
  () =>
    overviewNodes.value.filter(node => {
      ...
      const highUsage = [
        ...
        snapshot.diskUsage,
        snapshot.gpuUsage
      ].some(value => clampPercent(value) >= 80)
      return snapshot.online === false || snapshot.online === null || highUsage
    }).length
)
...
const onlineRate = computed(() => {
  ...
  if (!total) return 0
  return Math.round((onlineNodeCount.value / total) * 100)
})

const latestSnapshotTime = computed(() => {
  const timestamps = overviewNodes.value
    .map(node => node.snapshot.recordedAt || node.snapshot.timestamp)
    .filter(Boolean)
    .map(value => {
  ...
  return formatDateTime(Math.max(...timestamps), true)
})
...
const fetchNodes = async () => {
  ...
  try {
    ...
    if (
      ...
      !serverNodes.value.some(node => node.id === currentServerId.value)
    ) {
      currentServerId.value = ''
    ...
    nodes.map(async node => {
      try {
        ...
        return [
          node.id,
          existing
            ? { ...existing, online: false, errorMessage: error.message || '请求失败' }
            : {
                ...createEmptySnapshot(node),
                online: false,
...
const hydrateCurrentNodeSnapshot = async () => {
  ...
  const needsHydration =
    (Number(snapshot.memoryTotal) || 0) <= 0 || (Number(snapshot.diskTotal) || 0) <= 0
  ...
  try {
    ...
    if (
      (Number(next.memoryUsed) || 0) <= 0 &&
      (Number(next.memoryTotal) || 0) > 0 &&
      memoryUsage >= 0
    ) {
    ...
    if ((Number(next.gpuMemoryTotal) || 0) <= 0 && gpuMemoryTotal > 0)
      next.gpuMemoryTotal = gpuMemoryTotal
    if ((Number(next.gpuMemoryUsed) || 0) <= 0 && gpuMemoryUsed > 0)
      next.gpuMemoryUsed = gpuMemoryUsed
    const currentGpuMemoryText = String(next.gpuMemory || '')
      .trim()
      .toLowerCase()
    if (
      (!currentGpuMemoryText ||
        currentGpuMemoryText === 'n/a' ||
        currentGpuMemoryText === 'unknown') &&
      gpuMemoryTotal > 0
    ) {
    ...
    const currentGpuModel = String(next.gpuModel || '')
      .trim()
      .toLowerCase()
...
const fetchPrometheusStatus = async () => {
  try {
    ...
    hardwareAutoCollectEnabled.value =
