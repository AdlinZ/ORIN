...
# Snapshot covers Flyway migrations V1 through V87
#
# === Test-classpath copy (do not regenerate here) ===
# This file is a verbatim copy of docker/mysql/init/01-orin-schema.sql
# used by BaseIntegrationTest#withInitScript(...) so that the Testcontainers
# MySQL container starts from the V1..V87 baseline. Flyway is then configured
# with baseline-version=87 so it only applies V88..V95 on top of this snapshot.
# When the canonical docker/mysql/init snapshot is regenerated, refresh this
# copy in lockstep (or symlink if your build allows).
#
# --- Testcontainers workaround ---
# Testcontainers' ScriptUtils strips mysqldump conditional comments
# (/*!40014 ... */), which means the dump's original
# `SET FOREIGN_KEY_CHECKS=0` / `SET UNIQUE_CHECKS=0` are NOT applied. We
# therefore prepend plain SET statements here. Without these the dump fails
# with "Failed to open the referenced table 'gateway_services'" because
# gateway_routes is created before gateway_services in the dump ordering.
...
CREATE TABLE `agent_metadata` (
  ...
  `mcp_exposed` tinyint(1) NOT NULL DEFAULT '0',
  ...
  PRIMARY KEY (`agent_id`),
  KEY `idx_agent_metadata_owner_user_id` (`owner_user_id`),
  KEY `idx_agent_metadata_owner_mcp_exposed` (`owner_user_id`,`mcp_exposed`),
...
CREATE TABLE `gateway_secrets` (
  ...
  `secret_type` varchar(40) COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE `workflows` (
  ...
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_workflows_owner_mcp_exposed` (`owner_user_id`,`mcp_exposed`),
